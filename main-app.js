...
function notifyProjectsChanged() {
  try {
    if ('BroadcastChannel' in window) {
      new BroadcastChannel(projectsSyncChannel).postMessage({ type: 'projects-changed', at: Date.now() });
    }
  } catch (error) {
...
function setupProjectsSync() {
  try {
    if ('BroadcastChannel' in window) {
      const channel = new BroadcastChannel(projectsSyncChannel);
  ...
  window.addEventListener('storage', event => {
    if (event.key === projectsSyncChannel) {
      loadProjects();
    }
  });
}
